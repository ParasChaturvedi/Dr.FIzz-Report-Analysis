...
// For Claude AI prompt (keeps case)
...
/* ===========================
   NEW: convert Claude AI keywords into Optimize row format
   =========================== */
...
function buildClaudeRows(pxKeywords) {
...
export default function SeoAdvancedOptimize({
    ...
    (async () => {
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(msg || `Claude AI page-suggest failed (${res.status})`);
        }
        ...
        setPxKeywords([]);
        setPxError(e?.message || "Failed to fetch Claude AI keywords");
      } finally {
        if (!mounted) return;
        setPxLoading(false);
      }
  ...
  const keywords = useMemo(() => {
    // Per user's requirement: show ONLY Claude AI keywords in Optimize.
    // (No DataForSEO / seoData-derived keywords.)
    const pxRows = buildClaudeRows(pxKeywords);
