...
/* ============================================================
   Helpers for live opportunities API
============================================================ */
...
function mapApiItemToSlot(item, fallbackTitle, domain) {
  ...
  return {
    ...
    // prefer cached contentHtml if present
    content:
...
/* ============================================================
   Client-side cache (must match prefetch-opportunities.js)
============================================================ */
...
export default function OpportunitiesSection({ onOpenContentEditor }) {
  ...
  // keep a ref for the latest seoRows to avoid stale closure in async paths
  const seoRowsRef = useRef(null);
  useEffect(() => {
    seoRowsRef.current = seoRows;
  }, [seoRows]);

  const [prog, setProg] = useState(0);

  // load state for live API
  const [loadingOpps, setLoadingOpps] = useState(false);
  const [oppsError, setOppsError] = useState("");

  // scan/poll state (for 202 Accepted flow)
  ...
  useEffect(() => {
    ...
    const cachedClient = readOppsCache(d);
    if (cachedClient?.seoRows && alive) {
      ...
      // If cache says scan is still running, resume polling so UI updates when complete
      if (cachedClient.status === "running" && cachedClient.scanId) {
    ...
    const load = async ({ refetchAfterScan = false } = {}) => {
      ...
      try {
        ...
        if (alive) {
          ...
          if (!refetchAfterScan && !seoRowsRef.current && !cachedClient?.seoRows) {
            setSeoRows(null);
            setMultiRows(null);
          }
        }
