...
// Account popover with the signed-in user, role, credits, Profile settings,
// Log out, and — for admins — the Admin panel. Opened from the SIDEBAR Profile
// button (via auth context); anchored bottom-left next to the sidebar. No longer
// a floating bottom-right avatar.
"use client";
...
export default function AccountControl({ user, open, onClose, onLogout, onEditProfile }) {
  ...
  useEffect(() => {
    if (!open) return;
    const onDoc = (e) => {
      // Don't close when the click is the sidebar Profile trigger itself (it toggles).
      if (ref.current && !ref.current.contains(e.target) && !e.target.closest("[data-account-trigger]")) onClose?.();
    };
  ...
  return (
    <>
      <div
        ref={ref}
        className="fixed z-[80] [&_button]:cursor-pointer bottom-4 left-[64px] md:left-[80px] lg:left-[92px]"
