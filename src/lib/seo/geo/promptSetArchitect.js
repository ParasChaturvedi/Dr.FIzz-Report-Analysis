...
function buildIntake(source = {}, { size = 100 } = {}) {
  ...
  const themes = [...new Set([...themeVals(source.semanticThemes), ...themeVals(source.keywordClusters), ...themeVals(source.topicGaps)])].slice(0, 20);
 ...
 */
export async function generatePromptSet(source = {}, { size = 100, domain = "", timeoutMs = 200000 } = {}) {
  if (!String(process.env.ANTHROPIC_API_KEY || "").trim()) return null;
  const intake = buildIntake(source, { size });
  const messages = [
  ...
  const maxOut = Math.min(16000, Math.max(8000, Math.round(size * 140)));
  ...
  // Priority order = campaign priority; keep the model's within-campaign order.
  rows.sort((a, b) => ARCHITECT_CAMPAIGNS.indexOf(a.campaign) - ARCHITECT_CAMPAIGNS.indexOf(b.campaign));
  const prompts = rows.map((r, i) => ({
    prompt: r.prompt,
    cluster: r.campaign,
    campaign: r.campaign,
    topic: r.topic || "",
    ...
    quality_score: r.weight === "H" ? 95 : r.weight === "M" ? 80 : 65,
    priority_score: (ARCHITECT_CAMPAIGNS.length - ARCHITECT_CAMPAIGNS.indexOf(r.campaign)) * 100 + (r.weight === "H" ? 30 : r.weight === "M" ? 20 : 10),
    priority: i + 1,
